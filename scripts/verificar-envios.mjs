...
// no quede ningún dato residual. Se niega a ejecutarse contra Producción.
//
// Uso:
...
async function principal() {
  ...
  const debeContar = process.argv.includes("--contar");
  ...
  try {
    ...
    if (debeContar) {
      ...
      if (totalFilasAntes === 0) {
        bien("las 3 tablas de configuración tienen 0 filas antes de la prueba");
      } else {
        mal(
    ...
    try {
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (!intento.ok && intento.error?.code === "23505") {
          bien("1. Un municipio en dos zonas -> rechazado");
        } else {
          mal("1. Un municipio en dos zonas -> rechazado", intento.error?.message || "se insertó duplicado");
        }
      }
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (!intento.ok && intento.error?.code === "23505") {
          bien("2. Un departamento en dos zonas -> rechazado");
        } else {
          mal("2. Un departamento en dos zonas -> rechazado", intento.error?.message || "se insertó duplicado");
        }
      }
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (!intento.ok && intento.error?.code === "23P01") {
          bien("3. Dos tarifas publicadas solapadas en la misma zona -> rechazado por gist exclude");
        } else {
          mal(
            "3. Dos tarifas publicadas solapadas en la misma zona -> rechazado por gist exclude",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (conteo[0]?.n === 2 && t2[0]?.id) {
          bien("4. Periodos contiguos -> aceptados");
        } else {
          mal("4. Periodos contiguos -> aceptados", `se esperaban 2 tarifas publicadas, salieron ${conteo[0]?.n}`);
        }
      }
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (!intento.ok && intento.error?.message?.includes("Una tarifa publicada no cambia sus campos")) {
          bien("5. update del importe de una tarifa publicada -> rechazado por trigger inmutable");
        } else {
          mal(
            "5. update del importe de una tarifa publicada -> rechazado por trigger inmutable",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (!intento.ok && intento.error?.message?.includes("Una tarifa publicada no se despublica")) {
          bien("6. Despublicar una tarifa publicada -> rechazado");
        } else {
          mal(
            "6. Despublicar una tarifa publicada -> rechazado",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (
          ...
          bien("7. Cerrar vigente_hasta una vez -> aceptado; una segunda vez -> rechazado");
        } else {
          mal(
            "7. Cerrar vigente_hasta una vez -> aceptado; una segunda vez -> rechazado",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (hastaRechazado && futuraRechazada) {
          bien("8. Insertar publicada con vigente_hasta informado o vigente_desde futuro -> rechazado por trigger sin_programar");
        } else {
          mal(
            "8. Insertar publicada con vigente_hasta informado o vigente_desde futuro -> rechazado por trigger sin_programar",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (publicadaRechazada && borradorAceptado) {
          bien("9. Borrar una tarifa publicada -> rechazado; una nunca publicada -> aceptado");
        } else {
          mal(
            "9. Borrar una tarifa publicada -> rechazado; una nunca publicada -> aceptado",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (borradoRechazado && desactivacionAceptada) {
          bien("10. Borrar una zona con coberturas o tarifas -> rechazado; desactivarla -> aceptado");
        } else {
          mal(
            "10. Borrar una zona con coberturas o tarifas -> rechazado; desactivarla -> aceptado",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (borrado.rowCount === 1) {
          bien("11. Borrar una zona sin nada colgando -> aceptado");
        } else {
          mal("11. Borrar una zona sin nada colgando -> aceptado", `filas borradas: ${borrado.rowCount}`);
        }
      }
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (!intento.ok && intento.error?.code === "23503") {
          bien("12. Municipio de otro departamento en user_addresses -> rechazado por foreign key compuesta");
        } else {
          mal(
            "12. Municipio de otro departamento en user_addresses -> rechazado por foreign key compuesta",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (!intento.ok && intento.error?.code === "23514") {
          bien("13. Municipio con departamento nulo en user_addresses -> rechazado por check");
        } else {
          mal(
            "13. Municipio con departamento nulo en user_addresses -> rechazado por check",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (vigentes[0]?.n === 1 && cerradas[0]?.n === 2) {
          bien("14. Dos sustituciones concurrentes con for update -> queda una sola publicada vigente");
        } else {
          mal(
            "14. Dos sustituciones concurrentes con for update -> queda una sola publicada vigente",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (antesOk && despuesOk) {
          bien("15. audit_log recibe antes y despues en la misma transacción");
        } else {
          mal(
            "15. audit_log recibe antes y despues en la misma transacción",
      ...
      // -------------------------------------------------------------------------
      {
        ...
        if (nulos === 0 && invalidos === 0 && admins === total) {
          bien("16. Las cuentas existentes de admin_users quedan como administrador");
        } else {
          mal(
            "16. Las cuentas existentes de admin_users quedan como administrador",
    ...
    if (debeContar) {
      ...
      if (totalFilasDespues === 0) {
        bien("las 3 tablas de configuración tienen 0 filas tras la verificación (--contar)");
      } else {
        mal(
